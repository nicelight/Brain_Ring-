...
#ifndef NETWORK_SSID
#define NETWORK_SSID "Showmix"                   // Имя WiFi сети
#endif

#ifndef NETWORK_PASS
#define NETWORK_PASS "tol9igor"                   // Пароль для подключения к WiFi сети
#endif

#ifndef DEFAULT_IP

//~! 192, 168, 3, 203 Left Big Button
//~! 192, 168, 3, 204 Right Big Button
#define DEFAULT_IP {192, 168, 3, 204}     // Сетевой адрес устройства по умолчанию
#endif

char  udpBrainRingButt[11] = "=button204"; // если айпишник 203 то и тут "=button203" если айпишник 204 то тут "=button204"
uint16_t BrainRingServerPORT = 12345;
const char * BrainRingServerIP = "192.168.3.201"; // айпишник веб сервера на который отправляем UDP команду о нажатии кнопки -> 192.168.3.201
...
#ifndef GTW
#define GTW     1                         // Последняя цифра в IP адресе роутера. Обычно IP роутера 192.168.3.*1*, но некоторые роутеры имеют адрес 192.168.3.100 или 192.168.3.254
...
#define DEFAULT_AP_NAME    "PanelAP_1"             // Имя точки доступа по умолчанию 
...
#define WEATHER_REGION_YDX 65        // Код региона погоды по Yandex
#endif
...
enum  euatoBut  {INIT, BUTCHECK, SENDUDP, BUTUNHOLD} button_proc; // автомат опроса кнопки и отправки по UDP пакета на сервер 3.201
uint32_t prevButMs = 0, butDel = 100;
uint8_t butCounter = 0;
uint8_t ledblinkbut = 0; // автомат мигания светодиодом по нажатию на кнопку
uint32_t prevBlinkMs = 0; 
uint8_t blinkCounter = 0;
bool blinkTapButton = 0; // флаг что пора  бы помигать кнопкой
