          ...
          if (intData[1] == 1) {
            ...
            if (intData[1] == 2) {
              ...
              if (intData[1] == 3) {
                ...
                else if (thisMode == tmp_eff && tmp_eff == MC_TRAFFIC) {
...
//~ добавлен код для Brain Ring //!
void sendUDPtoBrainRing() {
  udp.beginPacket(BrainRingServerIP, BrainRingServerPORT);
  //or
  //  char charBuffer[5];
  //  memset(charBuffer, '\0', 5); // 5 bytes
  //  str.toCharArray(charBuffer, str.length() + 1);
  //  udp.write((const uint8_t*) charBuffer, str.length() + 1);
  //or
  //  udp.printf(str);
  //or
  udp.write((const uint8_t*) udpBrainRingButt, 11);
  udp.endPacket();
  udp.flush();
  delay(0);
  Serial.println(F("\n\n UDP to Brain Ring sent"));
}//sendUDPtoBrainRing()
